...
START_TEST (test_star_get_invalidedge) {
    ...
    setStar(&s, 0, 1, 10);
    setStar(&s, 0, 2, 7);
    ck_assert_int_eq(getCost(&s, 1, 2), -1);
    freeStar(&s);
}
...
Suite * star_suite(void)
{
    ...
    tcase_add_exit_test(tc_errors, test_star_get_invalidfrom, EXIT_FAILURE);
    tcase_add_exit_test(tc_errors, test_star_get_invalidto, EXIT_FAILURE);
